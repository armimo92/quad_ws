...
int main(int argc, char *argv[])
{
    ...
    { 
        ...
        thrust = (m/(cos(attitude(0))*cos(attitude(1)))) * (asmc(2)-g + z_ddot_des - (1/(xi_2(2)*gam(2)))*powf(std::abs(error_vel(2)), 2-gam(2))*sign(error_vel(2)) - (1/(xi_2(2)*gam(2)))*powf(std::abs(error_vel(2)), 2-gam(2))*sign(error_vel(2))*xi_1(2)*lambda(2)*powf(std::abs(error_pos(2)), lambda(2)-1));
